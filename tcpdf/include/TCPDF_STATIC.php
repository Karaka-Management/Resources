...
class TCPDF_STATIC {
	...
	public static function getObjFilename($type='tmp', $file_id='') {
		return \tempnam(K_PATH_CACHE, '__tcpdf_'.$file_id.'_'.$type.'_'.\md5(self::getRandomSeed()).'_');
	}
	 ...
	 * @public static
	 */
	public static function sendOutputData($data, $length) : void {
	...
	public static function getUserPermissionCode($permissions, $mode=0) {
		...
		foreach ($permissions as $permission) {
			if (isset($options[$permission])) {
				if (($mode > 0) || ($options[$permission] <= 32)) {
