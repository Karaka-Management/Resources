...
class TCPDF {
	...
	public function __construct($orientation = 'P', $unit = 'mm', $format = 'A4', $unicode = true, $encoding = 'UTF-8', $diskcache = false, $pdfa = false) {
		...
		$this->doc_creation_timestamp     = \time();
	 ...
	 */
	protected function setPageFormat($format, $orientation = 'P') : void {
		...
		if (\is_string($format)) {
			...
			if (isset($format['Rotate']) && (($format['Rotate'] % 90) == 0)) {
				...
				$this->pagedim[$this->page]['Rotate'] = (int) ($format['Rotate']);
			}
			if (isset($format['PZ'])) {
				// The page's preferred zoom (magnification) factor
				$this->pagedim[$this->page]['PZ'] = (float) ($format['PZ']);
			}
			if (isset($format['trans'])) {
				...
				if (isset($format['trans']['Dur'])) {
					...
					$this->pagedim[$this->page]['trans']['Dur'] = (float) ($format['trans']['Dur']);
				}
				...
				if (isset($format['trans']['S']) && \in_array($format['trans']['S'], $stansition_styles)) {
					...
					if (isset($format['trans']['Di']) && \in_array($format['trans']['S'], $valid_effect)) {
						if (((($format['trans']['Di'] == 90) || ($format['trans']['Di'] == 180)) && ($format['trans']['S'] === 'Wipe'))
							...
							$this->pagedim[$this->page]['trans']['Di'] = (int) ($format['trans']['Di']);
						}
					}
					if (isset($format['trans']['SS']) && ($format['trans']['S'] === 'Fly')) {
						$this->pagedim[$this->page]['trans']['SS'] = (float) ($format['trans']['SS']);
					}
				...
				if (isset($format['trans']['D'])) {
					...
					$this->pagedim[$this->page]['trans']['D'] = (float) ($format['trans']['D']);
				} else {
	 ...
	 * @param string     $orientation   page orientation. Possible values are (case insensitive):<ul><li>P or Portrait (default)</li><li>L or Landscape</li><li>'' (empty string) for automatic orientation</li></ul>
	 * @param bool|null  $autopagebreak boolean indicating if auto-page-break mode should be on or off
	 * @param float|null $bottommargin  bottom margin of the page
	 * @public
	 * @since 3.0.015 (2008-06-06)
	 */
	public function setPageOrientation($orientation, $autopagebreak = null, $bottommargin = null) : void {
	 ...
	 */
	public function setMargins($left, $top, $right = null, $keepmargins = false) : void {
	 ...
	 */
	public function setCellPaddings($left = null, $top = null, $right = null, $bottom = null) : void {
	 ...
	 * @since 6.4
	 */
	public function setAllowLocalFiles($allowLocalFiles) : void {
	 ...
	 * @since 4.6.021 (2009-07-20)
	 */
	protected function setContentMark($page = 0) : void {
		if ($page <= 0) {
			$page = $this->page;
		}
	 ...
	 * @public
	 */
	public function setHeaderData($ln = '', $lw = 0, $ht = '', $hs = '', $tc = [0,0,0], $lc = [0,0,0]) : void {
		$this->header_logo       = $ln;
	 ...
	 * @public
	 */
	public function setFooterData($tc = [0,0,0], $lc = [0,0,0]) : void {
	 ...
	 */
	protected function setTableHeader() : void {
	 ...
	 * @public
	 * @since 1.3
	 * @see SetFillColor[], SetDrawColor(), SetTextColor(), Rect(), Cell(), MultiCell()
	 */
	 ...
	 * @public
	 * @since 1.3
	 * @see SetTextColor[], SetDrawColor(), SetFillColor(), Text(), Cell(), MultiCell()
	 */
	 ...
	 * @since 4.0.013 (2008-07-28)
	 */
	protected function getFontsList() : void {
	...
	public function setFont($family, $style = '', $size = null, $fontfile = '', $subset = 'default', $out = true) : void {
		...
		if (isset($this->CurrentFont['fontkey'], $this->CurrentFont['subsetchars'])) {
	...
	public function setFontSize($size, $out = true) : void {
		...
		if (isset($this->CurrentFont['desc']['FontBBox'])) {
			...
			$font_height = (((int) ($bbox[3]) - (int) ($bbox[1])) * $size / 1000);
		} else {
	...
	public function getFontBBox() {
		...
		if (isset($this->CurrentFont['desc']['FontBBox'])) {
			...
			if (isset($this->CurrentFont['desc']['MaxWidth'])) {
				$maxw = $this->getAbsFontMeasure((int) ($this->CurrentFont['desc']['MaxWidth']));
			} else {
				$maxw = 0;
	...
	public function isCharDefined($char, $font = '', $style = '') {
		...
		if (TCPDF_STATIC::empty_string($font)) {
			if (TCPDF_STATIC::empty_string($style)) {
				return (isset($this->CurrentFont['cw'][(int) $char]));
			}
	 ...
	 */
	public function Annotation($x, $y, $w, $h, $text, $opt = ['Subtype' => 'Text'], $spaces = 0) : void {
		if ($this->inxobj) {
		...
		if (!$this->pdfa_mode || ($this->pdfa_mode && $this->pdfa_version == 3)) {
			if ((($opt['Subtype'] === 'FileAttachment') || ($opt['Subtype'] === 'Sound')) && (!TCPDF_STATIC::empty_string($opt['FS']))
				&& (@TCPDF_STATIC::file_exists($opt['FS']) || TCPDF_STATIC::isValidURL($opt['FS']))
	 ...
	 */
	public function Text($x, $y, $txt, $fstroke = 0, $fclip = false, $ffill = true, $border = 0, $ln = 0, $align = '', $fill = false, $link = '', $stretch = 0, $ignore_min_height = false, $calign = 'T', $valign = 'M', $rtloff = false) : void {
		...
		if ($ishtml) { // ******* Write HTML text
			...
			if ($maxh > 0) {
				...
				if ($fitcell && ($text_height > $maxh) && ($this->FontSizePt > 1)) {
					...
					$maxit        = (2 * \min(100, \max(10, (int) $fmax))); // max number of iterations
					while ($maxit >= 0) {
						$fmid = (($fmax + $fmin) / 2);
	...
	public function Image($file, $x = null, $y = null, $w = 0, $h = 0, $type = '', $link = '', $align = '', $resize = false, $dpi = 300, $palign = '', $ismask = false, $imgmask = false, $border = 0, $fitbox = false, $hidden = false, $fitonpage = false, $alt = false, $altimgs = []) {
		...
		if ($newimage) {
			...
			if ((\method_exists('TCPDF_IMAGES', $mtd)) && (!($resize && (\function_exists($gdfunction) || \extension_loaded('imagick'))))) {
				...
				if (($ismask === false) && ($imgmask === false) && (\strpos($file, '__tcpdf_'.$this->file_id.'_imgmask_') === false)
	 ...
	 * @see Image()
	 */
	protected function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $resize, $dpi, $palign, $filehash = '') : void {
		// create temp images
		if (empty($filehash)) {
			$filehash = \md5($file);
		}
	...
	public function _destroy($destroyall = false, $preserve_objcopy = false) : void {
		...
		foreach (\array_keys(\get_object_vars($this)) as $val) {
			if ($destroyall || !\in_array($val, $preserve)) {
				if ((!$preserve_objcopy || ($val != 'objcopy')) && ($val != 'file_id') && isset($this->{$val})) {
					unset($this->{$val});
				}
			}
		}
	 ...
	 * @protected
	 */
	protected function setPageBoxTypes($boxes) : void {
		$this->page_boxes = [];
	 ...
	 * @since 4.0.018 (2008-08-06)
	 */
	protected function _putannotsobjs() : void {
			...
			if (isset($this->PageAnnots[$n])) {
				...
				foreach ($this->PageAnnots[$n] as $key => $pl) {
					...
					if (\in_array(\strtolower($pl['opt']['subtype']), $markups)) {
						...
						//$annots .= ' /Popup ';
						if (isset($pl['opt']['ca'])) {
							$annots .= ' /CA '.\sprintf('%F', (float) ($pl['opt']['ca']));
						}
						if (isset($pl['opt']['rc'])) {
	 ...
	 */
	protected function _puttruetypeunicode($font) : void {
		$fontname = '';
		if ($font['subset']) {
	 ...
	 */
	protected function _putcidfont0($font) : void {
		$cidoffset = 0;
		if (!isset($font['cw'][1])) {
			$cidoffset = 31;
		}
	 ...
	 */
	protected function _putxobjects() : void {
		foreach ($this->xobjects as $key => $data) {
			if (isset($data['outdata'])) {
				$stream = \str_replace($this->epsmarker, '', \trim($data['outdata']));
	 ...
	 * @since 4.0.024 (2008-09-12)
	 */
	protected function _putspotcolors() : void {
	 ...
	 * @public
	 */
	public function setExtraXMPRDF($xmp) : void {
	...
	protected function _putviewerpreferences() {
		...
		if (isset($vp['NumCopies'])) {
			$out .= ' /NumCopies '.(int) ($vp['NumCopies']);
		}
		$out .= ' >>';
		return $out;
	}
	 ...
	 * @see getPageSizeFromFormat(), setPageFormat()
	 */
	protected function _beginpage($orientation = '', $format = '') : void {
		++$this->page;
	...
	public function setDocCreationTimestamp($time) : void {
		...
		$this->doc_creation_timestamp = (int) $time;
	}
	...
	public function setDocModificationTimestamp($time) : void {
		...
		$this->doc_modification_timestamp = (int) $time;
	}
	 ...
	 * @since 2.0.000 (2008-01-02)
	 */
	protected function _putencryption() : void {
		if (!$this->encrypted) {
			return;
		}
	 ...
	 * @author Nicola Asuni
	 */
	protected function _generateencryptionkey() : void {
	 ...
	 * @author Nicola Asuni
	 */
	public function setProtection($permissions = ['print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'], $user_pass = '', $owner_pass = null, $mode = 0, $pubkeys = null) : void {
		if ($this->pdfa_mode) {
	 ...
	 * @see StartTransform(), StopTransform()
	 */
	public function Scale($s_x, $s_y, $x = null, $y = null) : void {
		if (TCPDF_STATIC::empty_string($x)) {
			$x = $this->x;
		}
		if (TCPDF_STATIC::empty_string($y)) {
	 ...
	 * @see StartTransform(), StopTransform()
	 */
	public function MirrorH($x = null) : void {
	 ...
	 * @see StartTransform(), StopTransform()
	 */
	public function MirrorV($y = null) : void {
	 ...
	 * @see StartTransform(), StopTransform()
	 */
	public function MirrorP($x = null,$y = null) : void {
	 ...
	 * @see StartTransform(), StopTransform()
	 */
	public function MirrorL($angle = 0, $x = null,$y = null) : void {
		$this->Scale(-100, 100, $x, $y);
	 ...
	 * @see StartTransform(), StopTransform()
	 */
	public function Rotate($angle, $x = null, $y = null) : void {
		if (TCPDF_STATIC::empty_string($x)) {
			$x = $this->x;
		}
		if (TCPDF_STATIC::empty_string($y)) {
	 ...
	 * @see StartTransform(), StopTransform()
	 */
	public function SkewX($angle_x, $x = null, $y = null) : void {
	 ...
	 * @see StartTransform(), StopTransform()
	 */
	public function SkewY($angle_y, $x = null, $y = null) : void {
	 ...
	 * @see StartTransform(), StopTransform()
	 */
	public function Skew($angle_x, $angle_y, $x = null, $y = null) : void {
		if (TCPDF_STATIC::empty_string($x)) {
			$x = $this->x;
		}
		if (TCPDF_STATIC::empty_string($y)) {
	 ...
	 * @see StartTransform(), StopTransform()
	 */
	protected function Transform($tm) : void {
		if ($this->state != 2) {
			return;
		}
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	protected function _outPoint($x, $y) : void {
		if ($this->state == 2) {
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	protected function _outLine($x, $y) : void {
		if ($this->state == 2) {
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	protected function _outRect($x, $y, $w, $h, $op) : void {
		if ($this->state == 2) {
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	protected function _outCurve($x1, $y1, $x2, $y2, $x3, $y3) : void {
		if ($this->state == 2) {
	 ...
	 */
	protected function _outCurveV($x2, $y2, $x3, $y3) : void {
		if ($this->state == 2) {
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	protected function _outCurveY($x1, $y1, $x3, $y3) : void {
		if ($this->state == 2) {
	 ...
	 */
	public function Line($x1, $y1, $x2, $y2, $style = []) : void {
		if ($this->state != 2) {
			return;
		}
		if (\is_array($style)) {
	 ...
	 * @see SetLineStyle()
	 */
	public function Rect($x, $y, $w, $h, $style = '', $border_style = [], $fill_color = []) : void {
		if ($this->state != 2) {
			return;
		}
		if (empty($style)) {
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	public function Curve($x0, $y0, $x1, $y1, $x2, $y2, $x3, $y3, $style = '', $line_style = [], $fill_color = []) : void {
		if ($this->state != 2) {
			return;
		}
		if (!(\strpos($style, 'F') === false) && isset($fill_color)) {
			$this->setFillColorArray($fill_color);
		}
		$op = TCPDF_STATIC::getPathPaintOperator($style);
		if ($line_style) {
	 ...
	 */
	public function Polycurve($x0, $y0, $segments, $style = '', $line_style = [], $fill_color = []) : void {
		if ($this->state != 2) {
			return;
		}
		if (!(\strpos($style, 'F') === false) && isset($fill_color)) {
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	public function Ellipse($x0, $y0, $rx, $ry = 0, $angle = 0, $astart = 0, $afinish = 360, $style = '', $line_style = [], $fill_color = [], $nc = 2) : void {
		if ($this->state != 2) {
			return;
		}
	...
	protected function _outellipticalarc($xc, $yc, $rx, $ry, $xang = 0, $angs = 0, $angf = 360, $pie = false, $nc = 2, $startpoint = true, $ccw = true, $svg = false) {
		...
		// starting angle
		$ang      = $as;
		$alpha    = \sin($arcang) * ((\sqrt(4 + (3 * \pow(\tan(($arcang) / 2), 2))) - 1) / 3);
		$cos_xang = \cos($xang);
		$sin_xang = \sin($xang);
		$cos_ang  = \cos($ang);
		$sin_ang  = \sin($ang);
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	public function Circle($x0, $y0, $r, $angstr = 0, $angend = 360, $style = '', $line_style = [], $fill_color = [], $nc = 2) : void {
	 ...
	 * @public
	 */
	public function PolyLine($p, $style = '', $line_style = [], $fill_color = []) : void {
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	public function Polygon($p, $style = '', $line_style = [], $fill_color = [], $closed = true) : void {
		if ($this->state != 2) {
			return;
		}
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	public function RegularPolygon($x0, $y0, $r, $ns, $angle = 0, $draw_circle = false, $style = '', $line_style = [], $fill_color = [], $circle_style = '', $circle_outLine_style = [], $circle_fill_color = []) : void {
		if ($ns < 3) {
			$ns = 3;
		}
		if ($draw_circle) {
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	public function StarPolygon($x0, $y0, $r, $nv, $ng, $angle = 0, $draw_circle = false, $style = '', $line_style = [], $fill_color = [], $circle_style = '', $circle_outLine_style = [], $circle_fill_color = []) : void {
		if ($nv < 2) {
			$nv = 2;
		}
		if ($draw_circle) {
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	public function RoundedRect($x, $y, $w, $h, $r, $round_corner = '1111', $style = '', $border_style = [], $fill_color = []) : void {
	 ...
	 */
	public function RoundedRectXY($x, $y, $w, $h, $rx, $ry, $round_corner = '1111', $style = '', $border_style = [], $fill_color = []) : void {
		if ($this->state != 2) {
			return;
		}
	 ...
	 */
	protected function _putdests() : void {
		if (empty($this->dests)) {
			return;
		}
	 ...
	 * @public
	 */
	public function setBookmark($txt, $level = 0, $y = -1, $page = '', $style = '', $color = [0,0,0], $x = -1, $link = '') : void {
	 ...
	 * @since 2.1.002 (2008-02-12)
	 */
	public function Bookmark($txt, $level = 0, $y = -1, $page = '', $style = '', $color = [0,0,0], $x = -1, $link = '') : void {
		if ($level < 0) {
			$level = 0;
		}
	 ...
	 * @since 2.1.002 (2008-02-12)
	 */
	protected function _putjavascript() : void {
	 ...
	 * @since 2.1.002 (2008-02-12)
	 */
	protected function _addfield($type, $name, $x, $y, $w, $h, $prop) : void {
		if ($this->rtl) {
			$x = $x - $w;
		}
	 ...
	 */
	public function setFormDefaultProp($prop = []) : void {
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	public function TextField($name, $w, $h, $prop = [], $opt = [], $x = null, $y = null, $js = false) : void {
		if (TCPDF_STATIC::empty_string($x)) {
			$x = $this->x;
		}
		if (TCPDF_STATIC::empty_string($y)) {
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	public function RadioButton($name, $w, $prop = [], $opt = [], $onvalue = 'On', $checked = false, $x = null, $y = null, $js = false) : void {
		if (TCPDF_STATIC::empty_string($x)) {
			$x = $this->x;
		}
		if (TCPDF_STATIC::empty_string($y)) {
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	public function ListBox($name, $w, $h, $values, $prop = [], $opt = [], $x = null, $y = null, $js = false) : void {
		if (TCPDF_STATIC::empty_string($x)) {
			$x = $this->x;
		}
		if (TCPDF_STATIC::empty_string($y)) {
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	public function ComboBox($name, $w, $h, $values, $prop = [], $opt = [], $x = null, $y = null, $js = false) : void {
		if (TCPDF_STATIC::empty_string($x)) {
			$x = $this->x;
		}
		if (TCPDF_STATIC::empty_string($y)) {
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	public function CheckBox($name, $w, $checked = false, $prop = [], $opt = [], $onvalue = 'Yes', $x = null, $y = null, $js = false) : void {
		if (TCPDF_STATIC::empty_string($x)) {
			$x = $this->x;
		}
		if (TCPDF_STATIC::empty_string($y)) {
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	public function Button($name, $w, $h, $caption, $action, $prop = [], $opt = [], $x = null, $y = null, $js = false) : void {
		if (TCPDF_STATIC::empty_string($x)) {
			$x = $this->x;
		}
		if (TCPDF_STATIC::empty_string($y)) {
	 ...
	 */
	protected function _putsignature() : void {
		if ((!$this->sign) || (!isset($this->signature_data['cert_type']))) {
			return;
		}
	 ...
	 * @since 4.6.005 (2009-04-24)
	 */
	public function setSignature($signing_cert = '', $private_key = '', $private_key_password = '', $extracerts = '', $cert_type = 2, $info = [], $approval = '') : void {
		// to create self-signed signature: openssl req -x509 -nodes -days 365000 -newkey rsa:1024 -keyout tcpdf.crt -out tcpdf.crt
		// to export crt to p12: openssl pkcs12 -export -in tcpdf.crt -out tcpdf.p12
	 ...
	 * @since 3.0.000 (2008-03-27)
	 */
	public function startPageGroup($page = null) : void {
		if (empty($page)) {
			$page = $this->page + 1;
		}
	...
	public function setStartingPageNumber($num = 1) : void {
		$this->starting_page_number = \max(0, (int) $num);
	}
	 ...
	 * @since 3.0.000 (2008-03-27)
	 */
	protected function _putocg() : void {
		if (empty($this->pdflayers)) {
			return;
		}
	 ...
	 * @since 3.0.000 (2008-03-27)
	 */
	protected function setExtGState($gs) : void {
	 ...
	 * @since 5.9.152 (2012-03-23)
	 */
	public function setOverprint($stroking = true, $nonstroking = null, $mode = 0) : void {
		if ($this->state != 2) {
			return;
		}
	 ...
	 * @since 3.0.000 (2008-03-27)
	 */
	public function setAlpha($stroking = 1, $bm = 'Normal', $nonstroking = null, $ais = false) : void {
		if ($this->pdfa_mode && $this->pdfa_version < 2) {
			// transparency is not allowed in PDF/A-1 mode
			return;
		}
		$stroking = (float) $stroking;
		if (TCPDF_STATIC::empty_string($nonstroking)) {
			// default value if not set
			$nonstroking = $stroking;
		} else {
			$nonstroking = (float) $nonstroking;
		}
		if ($bm[0] === '/') {
	 ...
	 * @public
	 */
	public function cropMark($x, $y, $w, $h, $type = 'T,R,B,L', $color = [100,100,100,100,'All']) : void {
	 ...
	 * @public
	 */
	public function registrationMark($x, $y, $r, $double = false, $cola = [100,100,100,100,'All'], $colb = [0,0,0,0,'None']) : void {
	 ...
	 * @public
	 */
	public function LinearGradient($x, $y, $w, $h, $col1 = [], $col2 = [], $coords = [0,0,1,0]) : void {
		$this->Clip($x, $y, $w, $h);
	 ...
	 * @public
	 */
	public function RadialGradient($x, $y, $w, $h, $col1 = [], $col2 = [], $coords = [0.5,0.5,0.5,0.5,1]) : void {
		$this->Clip($x, $y, $w, $h);
	 ...
	 * @public
	 */
	public function CoonsPatchMesh($x, $y, $w, $h, $col1 = [], $col2 = [], $col3 = [], $col4 = [], $coords = [0.00,0.0,0.33,0.00,0.67,0.00,1.00,0.00,1.00,0.33,1.00,0.67,1.00,1.00,0.67,1.00,0.33,1.00,0.00,1.00,0.00,0.67,0.00,0.33], $coords_min = 0, $coords_max = 1, $antialias = false) : void {
		if (($this->pdfa_mode && $this->pdfa_version < 2) || ($this->state != 2)) {
			return;
		}
		$this->Clip($x, $y, $w, $h);
		...
		for ($i = 0; $i < $count_patch; ++$i) {
			...
			for ($j = 0; $j < $count_points; ++$j) {
				...
				$this->gradients[$n]['stream'] .= \chr((int) \floor((int) ($patch_array[$i]['points'][$j]) % 256));
			}
	 ...
	 * @protected
	 */
	protected function Clip($x, $y, $w, $h) : void {
		if ($this->state != 2) {
			 return;
		}
		if ($this->rtl) {
	 ...
	 * @public
	 */
	public function Gradient($type, $coords, $stops, $background = [], $antialias = false) : void {
		if (($this->pdfa_mode && $this->pdfa_version < 2) || ($this->state != 2)) {
			return;
		}
		$n                                   = \count($this->gradients) + 1;
	 ...
	 * @protected
	 */
	public function _putshaders() : void {
		if ($this->pdfa_mode && $this->pdfa_version < 2) {
			return;
		}
		...
		foreach ($this->gradients as $id => $grad) {
			if (($grad['type'] == 2) || ($grad['type'] == 3)) {
				...
				$out .= ' /Functions ['.\trim($functions).']';
				$out .= ' /Bounds ['.\trim($bounds).']';
				$out .= ' /Encode ['.\trim($encode).']';
				$out .= ' >>';
				$out .= "\n".'endobj';
				$this->_out($out);
				for ($i = 1; $i < $num_cols; ++$i) {
	...
	public function ImageEps($file, $x = null, $y = null, $w = 0, $h = 0, $link = '', $useBoundingBox = true, $align = '', $palign = '', $border = 0, $fitonpage = false, $fixoutvals = false) {
		...
		for ($i = 0; $i < $cnt; ++$i) {
			...
			$color_name = '';
			if (\strcasecmp('x', \substr(\trim($line), -1)) == 0) {
	 ...
	 * @public
	 */
	public function write1DBarcode($code, $type, $x = null, $y = null, $w = null, $h = null, $xres = null, $style = [], $align = '') : void {
		if (TCPDF_STATIC::empty_string(\trim($code))) {
			return;
		}
	 ...
	 * @public
	 */
	public function write2DBarcode($code, $type, $x = null, $y = null, $w = null, $h = null, $style = [], $align = '', $distort = false) : void {
		if (TCPDF_STATIC::empty_string(\trim($code))) {
			return;
		}
	...
	public function getCSSPadding($csspadding, $width = 0) {
		$padding      = \preg_split('/[\s]+/', \trim($csspadding));
	...
	public function getCSSMargin($cssmargin, $width = 0) {
		$margin      = \preg_split('/[\s]+/', \trim($cssmargin));
				...
				if (\preg_match('/type[\s]*=[\s]*"text\/css"/', $link, $type)) {
					...
					if (empty($type) || (isset($type[1]) && (($type[1] === 'all') || ($type[1] === 'print')))) {
						...
						if (\preg_match('/href[\s]*=[\s]*"([^"]*)"/', $link, $type) > 0) {
							// read CSS data file
                            $cssdata = $this->getCachedFileContents(\trim($type[1]));
		...
		while ($elkey < $maxel) {
			...
			if (\preg_match($tagpattern, $element)) {
				...
				if ($element[0] === '/') {
					...
					if (!empty($dom[$key]['value']) && ($dom[$key]['value'][0] === 'h') && ((int) ($dom[$key]['value'][1]) > 0) && ((int) ($dom[$key]['value'][1]) < 7)) {
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function writeHTML($html, $ln = true, $fill = false, $reseth = false, $cell = false, $align = '') : void {
		...
		while ($key < $maxel) {
			...
			if (($dom[$key]['value'] === 'tr') && isset($dom[$key]['thead']) && $dom[$key]['thead']) {
				if (isset($dom[$key]['parent'], $dom[$dom[$key]['parent']]['thead']) && !TCPDF_STATIC::empty_string($dom[$dom[$key]['parent']]['thead'])) {
					$this->inthead = true;
				...
				if ((!$this->newline) && ($dom[$key]['value'] === 'img') && (isset($dom[$key]['height'])) && ($dom[$key]['height'] > 0)) {
					...
					if (($fontname != $curfontname) || ($fontstyle != $curfontstyle) || ($fontsize != $curfontsize)
						...
						if (($key < ($maxel - 1)) && (
								($dom[$key]['tag'] && $dom[$key]['opening'] && ($dom[$key]['value'] === 'li'))
								|| ($this->cell_height_ratio != $dom[$key]['line-height'])
								|| (!$this->newline && \is_numeric($fontsize) && \is_numeric($curfontsize)
								&& ($fontsize >= 0) && ($curfontsize >= 0)
								&& (($fontsize != $curfontsize) || ($fontstyle != $curfontstyle) || ($fontname != $curfontname)))
							)) {
							if ($this->page > $startlinepage) {
			...
			if ($this->newline && (\strlen($dom[$key]['value']) > 0) && ($dom[$key]['value'] != 'td') && ($dom[$key]['value'] != 'th')) {
				...
				if (isset($startlinex)) {
					...
					if ($this->inxobj) {
						...
						if (isset($opentagpos, $this->footerlen[$startlinepage]) && (!$this->InFooter)) {
					...
					if ((isset($plalign) && ((($plalign === 'C') || ($plalign === 'J') || (($plalign === 'R') && (!$this->rtl)) || (($plalign === 'L') && ($this->rtl)))))) {
						...
						if ($this->isRTLTextDir()) { // RTL
							...
							if ($pos1 > 0) {
								$pos1 = (int) $pos1;
								if ($this->isUnicodeFont()) {
									$pos2     = (int) (TCPDF_STATIC::revstrpos($pmid, '[('.\chr(0).\chr(32)));
									$spacelen = 2;
								} else {
									$pos2     = (int) (TCPDF_STATIC::revstrpos($pmid, '[('.\chr(32)));
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
							...
							if ($pos1 > 0) {
								$pos1 = (int) $pos1;
								if ($this->isUnicodeFont()) {
									$pos2     = (int) (TCPDF_STATIC::revstrpos($pmid, \chr(0).\chr(32).')]')) + 2;
									$spacelen = 2;
								} else {
									$pos2     = (int) (TCPDF_STATIC::revstrpos($pmid, \chr(32).')]')) + 1;
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
									...
									switch ($strpiece[2][0]) {
										...
										case 'l': {
											...
											if (\preg_match('/([0-9\.\+\-]*)[\s]('.$strpiece[1][0].')[\s]('.$strpiece[2][0].')([\s]*)/x', $pmid, $pmatch) == 1) {
												$newpmid = \sprintf('%F',((float) ($pmatch[1]) + $spacew)).' '.$pmatch[2].' x*#!#*x'.$pmatch[3].$pmatch[4];
										...
										case 're': {
											...
											if (\preg_match('/('.$xmatches[1].')[\s]('.$xmatches[2].')[\s]('.$xmatches[3].')[\s]('.$strpiece[1][0].')[\s](re)([\s]*)/x', $pmid, $pmatch) == 1) {
												$newx    = \sprintf('%F',((float) ($pmatch[1]) + $x_diff));
												$neww    = \sprintf('%F',((float) ($pmatch[3]) + $w_diff));
										...
										case 'c': {
											...
											if (\preg_match('/('.$xmatches[1].')[\s]('.$xmatches[2].')[\s]('.$xmatches[3].')[\s]('.$xmatches[4].')[\s]('.$xmatches[5].')[\s]('.$strpiece[1][0].')[\s](c)([\s]*)/x', $pmid, $pmatch) == 1) {
												$newx1   = \sprintf('%F',((float) ($pmatch[1]) + $spacew));
												$newx2   = \sprintf('%F',((float) ($pmatch[3]) + $spacew));
												$newx3   = \sprintf('%F',((float) ($pmatch[5]) + $spacew));
		...
		switch($tag['value']) {
			...
			case 'input': {
				...
				if (isset($tag['attribute']['maxlength']) && !TCPDF_STATIC::empty_string($tag['attribute']['maxlength'])) {
					$opt['maxlen'] = (int) ($tag['attribute']['maxlength']);
				}
				...
				switch ($tag['attribute']['type']) {
					...
					case 'image': {
						...
						$value = 'img';
						//$opt['mk'] = ['i'=>$img, 'tp'=>1, 'if'=>array('sw'=>'A', 's'=>'A', 'fb'=>false)];
			...
			case 'textarea': {
				...
				if (isset($tag['attribute']['rows']) && !TCPDF_STATIC::empty_string($tag['attribute']['rows'])) {
					$h = (int) ($tag['attribute']['rows']) * $this->getCellHeight($this->FontSize);
				} else {
					$h = 10;
				}
				$prop['multiline'] = 'true';
	...
	protected function closeHTMLTagHandler($dom, $key, $cell, $maxbottomliney = 0) {
		...
		switch($tag['value']) {
			case 'tr': {
				...
				if (isset($dom[$table_el]['rowspans'])) {
					foreach ($dom[$table_el]['rowspans'] as $k => $trwsp) {
						--$dom[$table_el]['rowspans'][$k]['rowspan'];
				...
				if ($this->num_columns > 1) {
					if (($prev_page < $this->page)
						&& ((($this->current_column == 0) && ($dom[($dom[$key]['parent'])]['endcolumn'] == ($this->num_columns - 1)))
							|| ($this->current_column == $dom[($dom[$key]['parent'])]['endcolumn']))) {
						// page jump
						$this->selectColumn(0);
					...
					if (isset($dom[($dom[$key]['parent'])]['rowspans'])) {
						foreach ($dom[($dom[$key]['parent'])]['rowspans'] as $k => $trwsp) {
							...
							if ($dom[($dom[$key]['parent'])]['rowspans'][$k]['trid'] == $trkey) {
								--$dom[($dom[$key]['parent'])]['rowspans'][$k]['mrowspan'];
							}
						}
					}
	 ...
	 * @protected
	 */
	protected function addHTMLVertSpace($hbz = 0, $hb = 0, $cell = false, $firsttag = false, $lasttag = false) : void {
		if ($firsttag) {
			$this->Ln(0, $cell);
			$this->htmlvspace = 0;
			return;
	 ...
	 * @since 5.7.000 (2010-08-03)
	 */
	protected function drawHTMLTagBorder($tag, $xmax) : void {
	 ...
	 * @since 4.2.000 (2008-10-29)
	 */
	protected function swapMargins($reverse = true) : void {
		if ($reverse) {
	 ...
	 * $tagvs = ['h1' => array(0 => array('h' => '', 'n' => 2), 1 => array('h' => 1.3, 'n' => 1))];
	 ...
	 * @since 4.4.003 (2008-12-09)
	 */
	public function setHtmlLinksStyle($color = [0,0,255], $fontstyle = 'U') : void {
	 ...
	 * @since 4.2.010 (2008-11-14)
	 */
	protected function setGraphicVars($gvars, $extended = false) : void {
		if ($this->state != 2) {
			 return;
		}
	 ...
	 * @protected
	 */
	protected function _outSaveGraphicsState() : void {
		$this->_out('q');
	}
	 ...
	 * @protected
	 */
	protected function _outRestoreGraphicsState() : void {
		$this->_out('Q');
	}
	 ...
	 * @since 4.5.000 (2009-01-02)
	 */
	protected function setBuffer($data) : void {
	 ...
	 * @since 5.5.000 (2010-06-22)
	 */
	protected function replaceBuffer($data) : void {
		$this->bufferlen = \strlen($data);
		$this->buffer    = $data;
	}
	 ...
	 * @since 4.5.000 (2008-12-31)
	 */
	protected function setPageBuffer($page, $data, $append = false) : void {
		if ($append) {
	 ...
	 * @since 4.5.000 (2008-12-31)
	 */
	protected function setImageSubBuffer($image, $key, $data) : void {
	 ...
	 * @since 4.5.000 (2009-01-02)
	 */
	protected function setFontBuffer($font, $data) : void {
	 ...
	 * @since 4.5.000 (2009-01-02)
	 */
	protected function setFontSubBuffer($font, $key, $data) : void {
	 ...
	 */
	public function addTOC($page = null, $numbersfont = '', $filler = '.', $toc_name = 'TOC', $style = '', $color = [0,0,0]) : void {
	 ...
	 */
	public function addHTMLTOC($page = null, $toc_name = 'TOC', $templates = [], $correct_align = true, $style = '', $color = [0,0,0]) : void {
		$filler                  = ' ';
	 ...
	 * @since 4.9.001 (2010-03-28)
	 */
	public function setEqualColumns($numcols = 0, $width = 0, $y = null) : void {
		$this->columns = [];
		if ($numcols < 2) {
			$numcols       = 0;
			$this->columns = [];
	 ...
	 * @since 4.9.001 (2010-03-28)
	 */
	public function selectColumn($col = null) : void {
	 ...
	 * @public
	 */
	public function setTextShadow($params = ['enabled' => false, 'depth_w' => 0, 'depth_h' => 0, 'color' => false, 'opacity' => 1, 'blend_mode' => 'Normal']) : void {
		...
		if (isset($params['depth_w'])) {
			$this->txtshadow['depth_w'] = (float) ($params['depth_w']);
		} else {
			$this->txtshadow['depth_w'] = 0;
		}
		if (isset($params['depth_h'])) {
			$this->txtshadow['depth_h'] = (float) ($params['depth_h']);
		} else {
		...
		if (isset($params['opacity'])) {
			$this->txtshadow['opacity'] = \min(1, \max(0, (float) ($params['opacity'])));
		} else {
		...
		foreach ($unichars as $char) {
			if ((!$intag) && (!$skip) && TCPDF_FONT_DATA::$uni_type[$char] === 'L') {
				...
				if (\chr($char) === '<') {
					...
					$intag = false;
					// check for style tag
					$expected = [115, 116, 121, 108, 101]; // = 'styl'
	 ...
	 */
	public function printTemplate($id, $x = null, $y = null, $w = 0, $h = 0, $align = '', $palign = '', $fitonpage = false) : void {
		if ($this->state != 2) {
			 return;
		}
	 ...
	 */
	public function setPageRegions($regions = []) : void {
	...
	public function addPageRegion($region) : void {
		...
		if (isset($region['xt'], $region['xb']) && ($region['xt'] > 0) && ($region['xb'] > 0)
			&& isset($region['yt'], $region['yb']) && ($region['yt'] >= 0) && ($region['yt'] < $region['yb'])
	 ...
	 * @since 5.0.000 (2010-05-02)
	 */
	protected function SVGTransform($tm) : void {
		...
		if (\preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['fill'], $regs)) {
			...
			if ($gradient['mode'] === 'measure') {
				...
				foreach($gradient['coords'] as $key => $val) {
					$gradient['coords'][$key] = ((int) $val / 100);
					if ($val < 0) {
	...
	protected function SVGPath($d, $style = '') {
		...
		foreach ($paths as $key => $val) {
			...
			if (isset($val[2])) {
				...
				$rawparams = \preg_split('/([\,\s]+)/si', \trim($val[2]));
				$params    = [];
	 ...
	 * @protected
	 */
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm = []) : void {
		...
		if ($this->svgdefsmode && !\in_array($name, ['clipPath', 'linearGradient', 'radialGradient', 'stop'])) {
			if (isset($attribs['id'])) {
		...
		switch($name) {
					...
					if (\count($tmp) == 4) {
						...
						if (isset($attribs['preserveAspectRatio'])) {
							if($attribs['preserveAspectRatio'] === 'none') {
								...
								$tmp = $tmp[0];
								if ((\count($tmp) == 2) && (\strlen($tmp[0]) == 8) && (\in_array($tmp[1], ['meet', 'slice', 'none']))) {
			...
			case 'stop': {
				...
				if (\substr($attribs['offset'], -1) === '%') {
					$offset = (float) (\substr($attribs['offset'], 0, -1)) / 100;
				} else {
					$offset = (float) ($attribs['offset']);
					if ($offset > 1) {
						$offset /= 100;
					}
				}
			...
			case 'use': {
				if (isset($attribs['xlink:href']) && !empty($attribs['xlink:href'])) {
					...
					if (isset($this->svgdefs[$svgdefid])) {
						...
						if (isset($use['attribs']['x'], $attribs['x'])) {
							$attribs['x'] += $use['attribs']['x'];
						}
						if (isset($use['attribs']['y'], $attribs['y'])) {
	 ...
	 * @protected
	 */
	protected function endSVGElementHandler($parser, $name) : void {
		$name = $this->removeTagNamespace($name);
	 ...
	 * @protected
	 */
	protected function segSVGContentHandler($parser, $data) : void {
		$this->svgtext .= $data;
	}
    ...
    {
        if (isset($this->fileContentCache[$file]) || $this->getImageBuffer($file) !== false) {
            return true;
        }
